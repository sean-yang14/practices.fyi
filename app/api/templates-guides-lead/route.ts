...
const schema = z.object({
  ...
  practice_type: z.string().optional().default(""),
...
export async function POST(req: Request) {
  try {
    ...
    const { name, email, practice, practice_type, team_size, message } = parsed.data;

    // Log the submission (in production, you'd want to send an actual email)
    console.log("New Templates & Guides lead submission:");
    console.log({
      name,
      email,
      practice,
    ...
    if (process.env.RESEND_API_KEY) {
      ...
      await resend.emails.send({
        ...
        text: [
          ...
          `Email: ${email}`,
          `Practice: ${practice}`,
          `Practice Type: ${practice_type || "(not specified)"}`,
